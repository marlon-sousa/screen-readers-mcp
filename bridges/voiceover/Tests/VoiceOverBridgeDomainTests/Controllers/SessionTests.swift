...
struct SessionTests {
	...
	// ==========================================================================
	// 13.26: TEARDOWN PUTS THE MODIFIER BACK TOO -- AND AFTER THE VOICE.
	// ==========================================================================

	/// A handshake that also replaced the person's VoiceOver modifier, which is
	/// what `replacedModifier` records and what teardown owes a restart for.
	private func capsLockEdgeHandler(_ set: AdapterSet) -> FakeHandler {
		let handler = edgeHandler(set)
		let previous = handler.onExecute
		handler.onExecute = { context, request in
			previous?(context, request)
			context.replacedModifier = .capsLock
		}
		return handler
	}

	@Test("THE VOICE IS RESTORED BEFORE THE RESTART, and getting that backwards costs a voice")
	func theVoiceGoesBackBeforeTheReaderRestarts() {
		// Spec 0053 §3.1 said "the modifier, the voice" until this was implemented.
		// A restart RE-READS the voice selection, so restoring the modifier first
		// would restart VoiceOver while it was still on the capture voice -- 13.23's
		// hazard exactly: the reader comes up, cannot publish that voice, falls back
		// to the system default AND PERSISTS THE FALLBACK, and the record of the
		// person's own voice is destroyed by the cleanup rather than by the crash.
		let lifecycle = FakeProviderLifecycle()
		let restart = FakeReaderRestart()
		var voiceWasBackWhenItRestarted: Bool?
		restart.onRestart = { voiceWasBackWhenItRestarted = !lifecycle.restored.isEmpty }
		let set = fakeAdapterSet(providerLifecycle: lifecycle, readerRestart: restart)
		run(
			[hello(), request(2, "bye")],
			handlers: ["hello": capsLockEdgeHandler(set), "bye": byeHandler()])
		#expect(restart.restarts == 1)
		#expect(voiceWasBackWhenItRestarted == true)
	}

	@Test("a session that replaced nothing restarts nothing")
	func anOrdinarySessionRestartsNothing() {
		// The control, and the one that matters most: a teardown that restarted a
		// reader nothing had changed would take a blind person's screen reader away
		// at the end of every single session.
		let restart = FakeReaderRestart()
		let set = fakeAdapterSet(readerRestart: restart)
		run([hello(), request(2, "bye")], handlers: ["hello": edgeHandler(set), "bye": byeHandler()])
		#expect(restart.restarts == 0)
	}

	@Test("the human is told BEFORE the closing restart takes their reader away")
	func theClosingRestartIsAnnounced() {
		let announcer = FakeAnnouncer()
		let restart = FakeReaderRestart()
		let set = fakeAdapterSet(readerRestart: restart, announcer: announcer)
		run(
			[hello(), request(2, "bye")],
			handlers: ["hello": capsLockEdgeHandler(set), "bye": byeHandler()])
		#expect(announcer.spoken.last?.contains("restarting VoiceOver") == true)
	}

	@Test("A SESSION THAT CLEANED UP LEAVES NOTHING OPEN IN THE JOURNAL")
	func aCleanSessionClosesEveryChange() {
		// The journal's whole product is the OPEN change -- what a crashed session
		// left behind. So the assertion that gives that meaning is this one: an
		// ordinary ending closes everything it opened.
		let lifecycle = FakeProviderLifecycle()
		let restart = FakeReaderRestart()
		let journal = FakeChangeJournal()
		let set = fakeAdapterSet(
			providerLifecycle: lifecycle, readerRestart: restart, changeJournal: journal)
		let handler = capsLockEdgeHandler(set)
		let previous = handler.onExecute
		// Stand in for what the setup rung would have journalled.
		handler.onExecute = { context, request in
			previous?(context, request)
			journal.changed(ReaderEdgeSetup.voiceChange(was: "com.apple.eloquence.pt-BR.Reed"))
			journal.changed(ReaderEdgeSetup.runningModifierChange(was: .capsLock))
		}
		run([hello(), request(2, "bye")], handlers: ["hello": handler, "bye": byeHandler()])
		#expect(journal.openKinds.isEmpty)
	}

	@Test("a restart that FAILS at teardown leaves the change open and costs nothing else")
	func afailedClosingRestartIsRecorded() {
		// A blind person whose reader did not come back has to be able to find out
		// why, and the rest of teardown must still run -- the channel closes, the
		// transcript is closed, and the voice was already back.
		let restart = FakeReaderRestart()
		...
		let outcome = run(
			[hello(), request(2, "bye")], handlers: ["hello": handler, "bye": byeHandler()])
		#expect(journal.openKinds == [.runningModifier])
		#expect(outcome.transcript.notes.contains { $0.contains("would not come back") })
		#expect(lifecycle.restored == ["com.apple.eloquence.pt-BR.Reed"])
		#expect(outcome.channel.isClosed)
	}
