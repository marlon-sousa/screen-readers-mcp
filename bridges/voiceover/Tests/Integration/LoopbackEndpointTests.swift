// Headless integration: a session over a real loopback socket, the only place TCPBinder runs.
...
struct LoopbackEndpointTests {
	...
	func aSessionOverLoopback() throws {
		// Port 0 lets the kernel pick, so the reported endpoint must be the port bound.
