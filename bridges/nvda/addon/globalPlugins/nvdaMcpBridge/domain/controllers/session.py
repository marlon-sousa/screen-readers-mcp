...
class Session:
    ...
    def _teardown(self) -> None:
        ...
        # Cancel any outstanding prompt and resume speech, so a session that
        # dies with a window open leaves the tester audible. Safe even if hello
        # never ran (no prompt, no adapters).
        prompt = ctx.get_outstanding_prompt()
        if prompt is not None:
            prompt.cancel()
            ctx.clear_outstanding_prompt()
        if ctx.adapters is not None:
            self._guard(ctx.adapters.speech_source.resume)
