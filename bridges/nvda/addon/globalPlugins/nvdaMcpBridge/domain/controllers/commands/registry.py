...
# ROLE: the composition point for command handlers, a hand-written map of every wire command to its handler.
# BUILT BY: wiring.py in production; the test builder in unit tests.
# USED BY: the Session, which looks a command up and calls execute.
# Handlers are stateless singletons shared by every session; per-session state lives in the SessionContext.
