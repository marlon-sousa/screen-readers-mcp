...
whether something is there. The three tables in that section are the list, as
this machine has them bound — so it is the *rows* you must not reach for, whatever
keys they turned out to be on.
...
- **The reading commands** in the table above. They only re-read what is already
  in front of you, and a user presses them constantly.
