...
#
# The deadline is absolute (set once, never extended): an agent that stops polling
# cannot leave capture suspended indefinitely. The window's own lifetime is its
# own, not the poll's -- a poll miss leaves the window open, so present-then-poll
# survives transient failures. Only an answer, the deadline, or teardown closes it.
