...
# ROLE: adapter implementing MessageChannel over the Transport seam: line framing and JSON encoding.
# BUILT BY: wiring.serve_transport.
...
class JsonLinesChannel(MessageChannel):
	...
	def read_message(self) -> dict[str, Any] | Timeout:
		"""Drains a buffered line before reading, so an arrived message is never lost to a timeout.

		Raises ChannelClosed at EOF and protocol.ValidationError for an unreadable line.
		"""
		while True:
