...
# ROLE: leaf adapter implementing Transport over one connected named-pipe handle, plus the shared Win32
#       declarations.
# BUILT BY: named_pipe_listener.py for server instances, and dial() for clients.
# USED BY: adapters/json_lines_channel.py, through the Transport seam.
# A handle opened with FILE_FLAG_OVERLAPPED must use an OVERLAPPED structure for every ReadFile and WriteFile.
...
# A mismatched argtypes or restype below is a silent memory-safety bug, not a Python exception.

KERNEL32 = ctypes.WinDLL("kernel32", use_last_error=True)
ADVAPI32 = ctypes.WinDLL("advapi32", use_last_error=True)

#: ctypes unwraps a HANDLE return value to an int, so compare against this value, not -1.
...
#: The errors that mean the peer is gone, read as EOF.
...
def owner_only_security_attributes() -> SECURITY_ATTRIBUTES:
	"""The DACL grants access to the object's owner only (SDDL ``OW``); every other account is denied."""
...
class NamedPipeTransport(Transport):
	...
	def recv(self) -> bytes:
		"""``b""`` at EOF; TimeoutError when idle.

		A timed-out read is cancelled before raising, so it cannot complete later into an abandoned buffer.
		"""
		if self._closed:
			return b""
		...
		try:
			...
			if not ok:
				...
				if wait == WAIT_TIMEOUT:
					...
					# Drain the cancelled op so nothing stays pending on this handle.
	...
	def close(self) -> None:
		...
		# Fails harmlessly on a client handle.
...
def dial(pipe_name: str, *, timeout: float = DEFAULT_DIAL_TIMEOUT) -> NamedPipeTransport:
	...
	while True:
		...
		if err == ERROR_PIPE_BUSY:
			...
			# WaitNamedPipeW fails with ERROR_FILE_NOT_FOUND before the pipe exists, so back off and retry.
