...
def main() -> int:
	...
	print(
		...
		"spec 0022 (option (c)) it picks up nothing else it needs to: the tool list is "
		"a CONSTANT, so the cached copy this kill stranded is still a correct one.\n"
		"\n"
		"That is the whole of board entry 11.6, and it is why this message is now four "
		"lines shorter than it was. Killing the server still drops it out of the "
		"client's managed lifecycle, and the client still silently respawns it without "
		"re-running capability discovery -- but there is no longer anything to "
		...
		"but because the SERVER's own set of tools is not what it was when the client "
		"listed. Name the server: a bare `/mcp reconnect` does not take. Only the human "
		"at the keyboard can run it -- it is client UI, not anything an agent can "
		"reach.\n"
