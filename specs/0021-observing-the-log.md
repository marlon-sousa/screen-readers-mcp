...
The checks themselves; the **run** of them lives in the implementing PR's body as
checkboxes, per `AGENTS.md`. Every item is driven by `scripts/live_test.py`, so
re-running is one command per scenario rather than a session of reading things
off a screen: items 1–7 by `log`, item 6 both ways by `logerror` (the agent
causes the error) and `logwatch` (the human does), items 8–9 by `logsilent`.
...
1. Press a gesture, then `getLog` for that command alone: the slice now contains
   ...
   the check 11.4 could not pass. Verified 2026-08-01 — `executeGesture`,
...
3. Poll three times with the previous `nextPosition`: no record is returned twice,
   and none is skipped. Verified by stitching the three slices back together and
   comparing against a single read of the whole range.
...
6. `waitForLog { minLevel: "error", timeout: 30 }`, then provoke an error: it
   returns at the moment it happens, with a usable position. Verified both ways
   2026-08-01 — agent-caused (3.2 s) and human-caused (42.8 s of a 60 s window).
   The agent causes one by scheduling the error a few seconds out from NVDA's
   Python console: a blocked session cannot receive a command, but it can receive
   an **effect**, which is this entry's own insight applied to itself.
7. On a busy session, poll slower than the ring turns over: `truncated: true`
   ...
   DEBUG records that actually flood. Read back with `maxEntries` **above** the
   ring's capacity, so eviction is the only possible explanation: a cap and an
   eviction both say `truncated`, and they are different bugs for an agent —
...
9. In a **silent** session, take a speech entry's `logPosition` and read the
   ...
   braille viewer if no display is to hand — the braille half was **not run** on
   2026-08-01 for want of a display.
