...
Status: **agreed 2026-08-21.** Board entry **11.27**. Comes out of a question
asked during the 11.11/11.17 live run: *is the agent warned whether the machine
is unattended?*
...
| `bridges/nvda/addon/.../wiring.py`, `.../controllers/session.py`, `.../commands/session_context.py` | wiring + controller + parameter object (existing, **added to the layout**) | `attended` travels beside `silence_cap` at every hop, defaulting ATTENDED — the same safe direction `ATTENDED_DEFAULT` takes. Not foreseen when this table was written because the table stopped at the two ends and the value has to cross four. |
...
| `server/adapters/bridge/handshake_test.go` | unit (existing, **added**) | Two cases: a declared human survives a disagreeing cap, and a bridge that declares nothing leaves nil — the distinction a default would silently destroy. |
...
| `bridges/nvda/tests/unit/domain/controllers/test_session.py` | unit (existing) | **AMENDED from `test_hello.py`**: `hello`'s existing cap assertions live here, and splitting the pair across two files would let them drift. Three cases — attended declared, an empty room declared, and the two facts DISAGREEING. |
| `shared/tests/unit/test_protocol.py` | unit (existing, **added**) | The field is in `HelloResult`'s serialized set. The existing test enumerates every key exactly, so it fails on any addition — which is the point of it. |
| `bridges/nvda/tests/support/conformance_bridge.py` | harness (existing, **added**) | Declares the DISAGREEING pair — a human present, no cap — so the tier below tests something. An agreeing pair passes under either implementation and proves nothing. |
| `server/tests/conformance/` | conformance (existing) | The value crosses the real binary against the real bridge — the only tier where the Python that writes it meets the Go that reads it. Asserted on the SENTENCE rather than a raw field, because the sentence is all `connect_reader` returns. **Verified non-vacuous**: flipping the harness's declaration fails it. |
| `server/testsupport/bridge.go` | fake (existing, **added**) | `BridgeOptions.Attended`, nil by default — the only way to exercise the compatibility path, since the real bridge always declares. |
...
- **The one negation that remains is in the one file no automated tier reaches.**
  `attended = not unattended` now lives in `plugin.py`, the composition root at
  the NVDA edge, which imports NVDA and wx and so is exercised by nothing headless
  — the same is already true of the `SilenceCapPolicy.from_settings` call beside
  it. Both ends of that line are pinned (`IniBridgeConfig.get_unattended` has unit
  tests; everything downstream of `build_session` has unit and conformance tests),
  but the `not` itself is covered only by the LIVE CHECKLIST on the implementing
  PR: tick the control dialog's *unattended* box, connect, and read the sentence.
  Worth saying plainly in an entry about an inversion being in the wrong place.
...
## Open questions — **all three closed on agreement, 2026-08-21**

- **`attended` or `unattended` on the wire? → `attended`.** A positive assertion
  reads better in a schema, and it puts the field's dangerous value where a
  mistake is loud: `false` is an explicit claim that the room is empty, so a
  bridge that forgets to fill the field gets *absent* — the third answer, which
  falls back to the inference — rather than an accidental empty room. The single
  negation lives on the bridge at the config read, beside the setting, pinned by
  a unit test.
- **Should the cap's `enabled` stay derived on the bridge? → Yes, derived.**
  The bridge keeps ONE setting. `attended` and `enabled` are both computed from
  `unattended` at the same point, side by side, which is the whole shape of Part
  3.2: two facts from one source rather than one from the other. Splitting them
  means a second checkbox in the control dialog — a config decision for the human
  at the reader, and a different entry if anyone ever wants it. Note that this
  leaves the first of Part 1's three scenarios *expressible but not yet fixed*:
  a present user who wants no cap still cannot say so. Making it expressible is
  this entry; offering them the switch is not.
- **Does `status` report attendance too? → No, connect only.** Attendance is
  fixed for the session, like mode and persona: it cannot change under the
  agent's feet, so restating it on every status round trip is weight for no news.
  It also matches today — `silenceCap` has always been a `connect_reader` field
  and `status` has never carried it.
