...
bring the reader back. **The control dialog is 13.14, not 13.10**: it was split out on 2026-08-30 so that it can be checked by driving the reader over its own window rather than by eye, and until then a bridge is started by `BridgeListener`, which reads the same persisted settings the dialog will edit. Its `pyproject.toml` carries **no Python**: it is the declaration file `scripts/bridges.py` reads, and [spec 0046](specs/0046-the-voiceover-bridge-class-by-class.md) names that a wart and declines the cheap fix. | Swift 6 (macOS only) |
