...
| `bridges/voiceover/` | The macOS VoiceOver bridge: one Swift `.app`, built by `build.sh` because SwiftPM cannot emit bundles. It holds the **capture voice** — a speech synthesis provider macOS hands every utterance as SSML — **`Sources/ScreenReaderWire/`, the contract's third binding** (hand-written Swift value types gated against `specs/wire/v1/schema.json` by `scripts/drift.py --swift`), and, since 13.4, **the bridge session**: `VoiceOverBridgeDomain` (ports, `Session`, handlers, entities) and `VoiceOverBridgeAdapters` (the two listeners, the JSON-lines channel, `Wiring`). It **listens** on the local endpoint the server dials, and since 13.5 it **hears**: the capture voice appends JSON lines to a file in its own container — the only door out of a sandboxed speech provider — and `ContainerFileSpeechSource` tails it into the session's `SpeechBuffer`. Since 13.6 it can make the reader **quiet** — silent sessions are established rather than refused, the bridge selects the capture voice itself and restores the user's own on every teardown path, and the silence it holds is a **lease that expires**, which is hard invariant 3 in its macOS form — and since 13.7 it can **drive** the reader: `pressGesture` dispatches VoiceOver's own English command names through the reader's `commander object`, asking for no Accessibility grant. 13.8 adds the **other half of input** — `typeText` synthesizes keystrokes into whatever holds focus — and with it the lane's one design lever: the two halves cost **different** permissions, so the Accessibility grant is requested on the first `typeText` of a session and from nowhere else, which makes *"a session that only presses commands and reads speech never asked for Accessibility"* a statement the tests check. **13.17 narrows that sentence by one word and is the entry to read for what the lever is now**: `pressGesture` also takes **keystrokes** — `command+l`, which a blind user presses constantly and which this bridge simply could not send, a gap that sat unnoticed through four entries because it had been described as a limit of the platform — so the grant has a second caller, both are command handlers about to post a system event, and the claim reads *"a session that presses only the reader's COMMAND NAMES and reads speech"*. Its hard part is the **keyboard layout**: which key produces `l` is the active layout's answer, read through `TISCopyCurrentKeyboardInputSource`/`UCKeyTranslate` and never a hard-coded table, and a character the layout cannot reach is a **named failure** rather than a wrong key. **13.19 then found the hole 13.17's rule left** -- with single-key Quick Nav on, an ordinary user presses `h` to move by heading, and the `+` rule made a lone token a command name, so the letter key had no notation at all. It has one now: a `kb:` **source prefix**, which is NVDA's own and which `protocol.md` §5 stops calling legacy, so `kb:h` is the key and `h` is still a command name, and the lever's sentence survives word for word because a keystroke costs the grant with or without modifiers. That entry also **standardized the key names on lane 1's**, read out of `../nvda/source/vkCodes.py`: every name this bridge accepts means the same physical key on the other reader, and one that would not -- `delete`, which is erases-backwards here and erases-forwards there -- is refused BY NAME rather than pressed. **13.22 turns the key into keys**: `kb:leftArrow+rightArrow` is arrow-key Quick Nav, the chord an ordinary user presses to turn on a mode they navigate with all day, and the notation had no way to say it -- so a keystroke is now modifiers plus one or MORE keys, with no second separator and no cap. Whether the reader accepts SYNTHESIZED simultaneity was measured rather than assumed (the two arrows sequentially move nothing; together they toggle it; no delay is needed), the release generalises spec 0048's `defer` so a press that fails partway cannot leave an ordinary key held down, and 13.8's lever survives untouched because a chord is a `CGEvent` like any other. The same measurement is why the command name stayed the RECOMMENDED route at that entry: the chord moves two settings and announces neither. **13.25 ended that recommendation, and the lever with it — read it before quoting either.** 13.9 adds **focus** — `getFocusInfo` reads the accessibility tree where that grant exists and VoiceOver's own cursor where it does not, and **never asks for the grant itself**, which is the lever surviving the entry that wanted it: the route is chosen through a one-method adapter seam that shows no dialog, so the object focus holds cannot request anything. 13.10 adds the **human channel**: `announce` speaks with the bridge's own synthesizer, **outside VoiceOver entirely**, so it is audible in the one mode where the reader is mute — which is what makes `pressGesture`'s and `typeText`'s `announce` field honourable at all — and `askUser` / `waitForUserReply` put a question to the person and collect the answer later, polled rather than awaited because the thread that would block is the one renewing the silence lease. 13.11 closes the lane's capability set with **`guidance`** — this reader's own account of the stance a session declared, five `.md` files resolved through a `Bundle.module` resource bundle, which is the contract's **third rendering of the embedded-document trap** — and packages it: `poe build` assembles the `.app`, `poe conformance` now drives the real Go binary against the real **Swift** bridge as well as the real Python one (the first time this lane's two halves are gated together), and `server/config/defaults.json` ships the reader so no agent has to configure one. So `hello` announces `speech`, `gestures`, `typing`, `focus`, `interact` and `guidance` — **six of the contract's eleven, and the five it omits are absent from the READER**, which is the capability gate visibly doing its job. **13.20 is where `connect_reader` stopped
lying about the machine**: `hello` used to REPORT where the ProviderState ladder
had stopped -- refusing a silent session, letting a live one through with a note
-- and a live session on a machine that could not capture then answered
`speech: []`, which is indistinguishable from "the reader said nothing" and cost
an hour of 13.19's live checklist. It now CLIMBS the ladder instead: it reads
both permissions (never requesting one, so 13.8's lever survives word for word),
starts the reader if it is not running, registers the extension the system forgot
the last time `poe build` ran, selects the voice, and then makes the reader speak
and requires the utterance to ARRIVE -- failing by named rung in BOTH modes,
because the promise this establishes is that `getSpeech` means anything at all
and a live session makes it as loudly as a silent one. Its rule is one sentence:
**session state is restored at teardown -- the voice selection -- and machine
state is not**, so there is deliberately no `unregister()`. The one rung it
cannot climb is `registered` to `published`, because only a reader RESTART
publishes a newly registered voice and no handshake may restart a blind person's
screen reader; that failure names the restart and spells it as a pair, `killall
VoiceOver && open -a VoiceOver`, because `killall` alone was measured not to
bring the reader back. **13.25 is where the agent started pressing what a blind user presses, and it is
the entry that changes what a session IS.** A VoiceOver user reaches the menu bar
with VO-M; this bridge could only send `go to menu bar`, dispatched INSIDE the
reader over an AppleEvent, which never passes the application under test — so an
application that swallows or reinterprets the chord reported success while a real
user was stuck, and the route the guidance recommended was hiding the defect class
the tool exists to find. `vo` is now a modifier in the notation that already
existed, RESOLVED FROM THE MACHINE (`SCRKeysToUseForVOModifier`, read through a
port, per call) exactly as lane 1's `NVDA` is resolved by NVDA — with Caps-Lock-only
and unreadable staying NAMED FAILURES that press nothing, because what 13.19
refused was guessing and guessing is still refused. **The lever is the price**:
*"a session that presses only the reader's COMMAND NAMES and reads speech is never
asked for Accessibility"* now describes a READING-ONLY session and not ordinary
driving, because a faithful user-persona session presses keys — a lever bought by
driving the reader in a way no user does is bought with the fidelity this tool
...
what let the guidance be rebuilt on measured keys rather than remembered ones and
retires 13.7's "no table of key combinations" — and **this reader matches bindings
on the event's CHARACTER**, so `control+option+shift+q` carrying `q` reached VO-Q
rather than VO-Shift-Q and reported success. That was a live defect in the shipped
presser, fixed by stamping the character the active layout produces on the layer
being pressed. Spec 0052. **The control dialog is 13.14, not 13.10**: it was split out on 2026-08-30 so that it can be checked by driving the reader over its own window rather than by eye, and until then a bridge is started by `BridgeListener`, which reads the same persisted settings the dialog will edit. Its `pyproject.toml` carries **no Python**: it is the declaration file `scripts/bridges.py` reads, and [spec 0046](specs/0046-the-voiceover-bridge-class-by-class.md) names that a wart and declines the cheap fix. | Swift 6 (macOS only) |
...
uv run poe conformance   # the real Go binary against each real bridge: the
                         # Python one everywhere, and the SWIFT one on macOS
