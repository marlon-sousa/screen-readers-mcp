...
// --reader; never by the namespace scanner. Spec 0013 is explicit that an endpoint
...
// between the local endpoint and loopback TCP, so a single endpoint would be
// wrong whenever they had; trying them in declared order makes the agent's
// model "connect to
